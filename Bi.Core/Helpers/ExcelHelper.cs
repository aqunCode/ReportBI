...
namespace Bi.Core.Helpers
{
    ...
    public class ExcelHelper
    {
                    ...
                    if (!attribute.Format.IsNullOrEmpty())
                    {
                        ...
                        else if (format[0] == "image")
                        {
                            ...
                            for (int row = 2; row <= sheet.Dimension.End.Row; row++)
                            {
                                ...
                                {
                                    //加载图片
                                    //var image = Image.Load(imgeBytes);

                                    ExcelImage image = new ExcelImage(imgeBytes, ePictureType.Jpg);

                                    //设置行高
                                    sheet.Row(row).Height = image.Bounds.Height;

                                    using (var imgStream = new MemoryStream(imgeBytes))
                                    {
                                        //添加图片
                                        var pic = sheet.Drawings.AddPicture($"image_{DateTime.Now.Ticks}", imgStream, ePictureType.Jpg);
                                        //设定图片位置
                                ...
                                //{
                                //    //加载图片
                                //    //注意：从.net6.0开始System.Drawing.Common不再支持跨平台，详情：https://docs.microsoft.com/zh-cn/dotnet/core/compatibility/core-libraries/6.0/system-drawing-common-windows-only#see-also
        ...
        public static byte[] EPPlusExportExcelToBytes<T1, T2, T3>(IEnumerable<T1> list1, IEnumerable<T2> list2 = null, IEnumerable<T3> list3 = null, string shtname1 = null, string shtname2 = null, string shtname3 = null, Action<ExcelWorksheet> action = null, TableStyles? styles = TableStyles.Light1) where T1 : class, new() where T2 : class, new() where T3 : class, new()
        {
            using var package = new ExcelPackage();

            if (list1?.Count() > 0)
            {
                using var sheet = package.Workbook.Worksheets.Add(shtname1 ?? "paga1");
                using var sheet2 = package.Workbook.Worksheets.Add(shtname2 ?? "paga2");
                using var sheet3 = package.Workbook.Worksheets.Add(shtname3 ?? "paga3");

                sheet.Cells["A1"].LoadFromCollection(list1, true, styles);
                //单元格自动适应大小
                sheet.Cells.AutoFitColumns();
                //单独设置单元格
                action?.Invoke(sheet);
                ...
                if (list2 != null && list2?.Count() > 0)
                {
                    ...
                    sheet2.Cells["A1"].LoadFromCollection(list2, true, styles);
                    //单元格自动适应大小
                    sheet2.Cells.AutoFitColumns();
                    //单独设置单元格
                    action?.Invoke(sheet2);
                }
                //
                if (list3 != null && list3.Count() > 0)
                {

                    sheet3.Cells["A1"].LoadFromCollection(list3, true, styles);
                    //单元格自动适应大小
                    sheet3.Cells.AutoFitColumns();
                    //单独设置单元格
                    action?.Invoke(sheet3);
                }
                //
                return package.GetAsByteArray();
            }
    ...
    public class ExcelHeaderCell
    {
        ...
        /// 字体颜色，默认黑色
        /// </summary>
        public Color FontColor { get; set; } = Color.Black;//eThemeSchemeColor.Background1;
