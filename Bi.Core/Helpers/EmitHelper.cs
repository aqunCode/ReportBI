...
namespace Bi.Core.Helpers
{
    ...
    public static class EmitHelper
    {
        ...
        /// <param name="value">数值</param>
        /// <returns></returns>
        public static ILGenerator Ldarg(this ILGenerator IL, int value)
        {
            switch (value)
            {
                case 0:
        ...
        /// <param name="value">数值</param>
        /// <returns></returns>
        public static ILGenerator Stloc(this ILGenerator IL, int value)
        {
            switch (value)
            {
                case 0:
        ...
        /// <param name="value">数值</param>
        /// <returns></returns>
        public static ILGenerator Ldloc(this ILGenerator IL, int value)
        {
            switch (value)
            {
                case 0:
            ...
            else
            {
                switch (Type.GetTypeCode(type))
                {
                    ...
                    case TypeCode.Double:
                        ...
                        break;
                    default:
                        throw new Exception(string.Format("{0}不支持的类型{1}", "Ldind", type));
                }
            }
            return IL;
        }
            ...
            else
            {
                switch (Type.GetTypeCode(type))
                {
                    ...
                    case TypeCode.Double:
                        ...
                        break;
                    default:
                        throw new Exception(string.Format("{0}不支持的类型{1}", "Stind", type));
                }
            }
            return IL;
        }
        ...
        {
            // 声明目标类型的本地变量
            //int index = IL.DeclareLocal(type).LocalIndex;
        ...
        /// <param name="method"></param>
        /// <returns></returns>
        public static ILGenerator PushParams(this ILGenerator IL, int paramIndex, MethodBase method)
        {
            var ps = method.GetParameters();
            for (var i = 0; i < ps.Length; i++)
            {
        ...
        /// <returns></returns>
        public static ILGenerator PushParams(this ILGenerator IL, int paramIndex, Type[] paramTypes)
        {
