...
public static class EncodingHelper
{
    ...
    static Encoding DetectUnicode(byte[] data)
    {
        ...
        if (((evenBinaryNullsInSample * 2.0) / data.Length) < 0.2 && ((oddBinaryNullsInSample * 2.0) / data.Length) > 0.6)
            return Encoding.Unicode;
        if (((oddBinaryNullsInSample * 2.0) / data.Length) < 0.2 && ((evenBinaryNullsInSample * 2.0) / data.Length) > 0.6)
    ...
    /// <returns></returns>
    private static int DetectSuspiciousUTF8SequenceLength(byte[] buf, long pos)
    {
        if (buf.Length > pos + 1)
        {
            var first = buf[pos];
