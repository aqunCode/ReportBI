...
namespace Bi.Core.Caches
{
    ...
    public class StackExchangeRedisCache : ICache
    {
        ...
        {
            var res = new List<T>();
            var keys = await KeysLikeAsync(patternKey);
        ...
        /// <param name="patternKey"></param>
        /// <returns></returns>
        public async Task<string[]> KeysLikeAsync(string patternKey)
        {
            ...
            bool addPrefix = true)
        {
            if(addPrefix)
                key = AddKeyPrefix(key);
            return await Redis.SortedSetRangeByScoreWithScoresAsync(key, addPrefix);
        }
        ...
        {
            key = AddKeyPrefix(key);
            return await Redis.ListRightPushAsync(key, redisValue);
        }
        #endregion
