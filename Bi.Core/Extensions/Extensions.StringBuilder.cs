...
namespace Bi.Core.Extensions
{
    ...
    public static class StringBuilderExtensions
    {
        #region LineCount
        /// <summary>
        /// 这个LineCount方法的功能是统计StringBuilder对象中的行数，其中使用了IndexOf和Environment.NewLine查找每个换行符的位置，并计算换行符的数量
        /// </summary>
        /// <param name="builder"></param>
        /// <returns></returns>
        public static int LineCount(this StringBuilder builder)
        {
            int count = 0;
            int position = 0;
            while ((position = builder.IndexOf(Environment.NewLine, position)) != -1)
            {
                count++;
